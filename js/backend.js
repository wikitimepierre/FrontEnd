		...
		.then (
			response => {
				...
				if (response.status === 204) {																	// Token good
					...
					alert("effacé du serveur : n°"+i+", id:"+id+" - "+title);	// CNSL
				...
				else if (response.status === 401) {alert("Pas d'autorisation")} // Token PAS good
				else {alert("Erreur "+response.status)}													// Token PAS good
			}
		)
	}
}